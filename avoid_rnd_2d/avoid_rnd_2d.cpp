   ...
   for(int i=0;i<N;++i)
	 ...
	 //exit(-1);}//no hay mas espacios hacia donde moverse por lo tanto debo terminar el programa
	   ...
	   for(int k=0;k<j;k++)
		 ...
		 {
		   if(X1[j+i*Steps]==X1[k+i*Steps] && X2[j+i*Steps]==X2[k+i*Steps]) //Reviso si la posicion siguiente a X1 de j corresponde a la poscion k-esima de X1 (con esto comparo las parejas (X1[j]+1,X2[j]) y la posicion ya ocupada (X1[k],X2[k]) )
		     {
		       j=0;
		       break;
		     }
		 ...
		 {
		   if(X2[j+i*Steps]==X2[k+i*Steps] && X1[j+i*Steps]==X1[k+i*Steps])
		     {
		       j=0;
		       break;
		     }
